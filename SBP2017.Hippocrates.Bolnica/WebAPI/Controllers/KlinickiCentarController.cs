...
{
    public class KlinickiCentarController : ApiController
    {
        public IHttpActionResult GetAll()
        {
            IList<KlinickiCentarDto> listDto = KlinickiCentarProvider.GetAll();
            if (listDto.Count == 0)
        ...
        public IHttpActionResult Get(int id)
        {
            ...
            bool found = true;
            KlinickiCentarDto d = KlinickiCentarProvider.Get(id, out found);
            if (!found)
        ...
        public IHttpActionResult Post([FromBody]KlinickiCentarDto value)
        {
            string s = "";
            if (KlinickiCentarProvider.Add(value, out s))
                return Content(HttpStatusCode.Created, s);
            return Content(HttpStatusCode.BadRequest, s);

        }

        public IHttpActionResult Put(int id, [FromBody]KlinickiCentarDto value)
        {
            string st = "";
            if (KlinickiCentarProvider.Update(id, value, out st))
        ...
        {
            string st = "";
            if (KlinickiCentarProvider.Delete(id, out st))
                return Content(HttpStatusCode.OK, st);
            else
                return Content(HttpStatusCode.BadRequest, st);
        }
